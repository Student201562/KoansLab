...
public class AboutStreams {
    ...
    public void simpleCount() {
        long count = places.stream().count(); //Поток представляет собой последовательность элементов 
												//и поддерживает различные виды операций для выполнения вычислений:
    ...
    public void mapReduce() {
        ...
        assertEquals(averageLength, 8.0d); // Средняя длина слов
    }
    ...
    public void limitSkip() {
        int lengthSum_Limit_3_Skip_1 = places.stream()
                .mapToInt(String::length)
                .limit(3) //Метод skip(long n) используется для пропуска n элементов. Этот метод возвращает новый поток, в котором пропущены первые n элементов.
