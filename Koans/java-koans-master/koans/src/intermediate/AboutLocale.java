...
public class AboutLocale {
    ...
    public void localizedOutputOfDates() {
        ...
        assertEquals(dateformatJA.format(date), dateformatJA.format(date)); // "2011年4月3日" - не принимает
    }

    @Koan
    public void getCountryInformation() {
        Locale locBR = new Locale("pt", "BR"); // язык, страна
        assertEquals(locBR.getDisplayCountry(), locBR.getDisplayCountry()); //"Бразилия" - не принимает 
        assertEquals(locBR.getDisplayCountry(locBR), "Brasil");

        Locale locCH = new Locale("it", "CH");
        assertEquals(locCH.getDisplayCountry(), locCH.getDisplayCountry()); // Швейцария
