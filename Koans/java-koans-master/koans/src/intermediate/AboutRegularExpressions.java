...
public class AboutRegularExpressions {
    ...
    public void basicMatching() {
        ...
        // index 012345678
        assertEquals(m.find(), true); // matchesМетод пытается согласовать всю входную последовательность по отношению к шаблону.
								 // lookingAtМетод пытается соответствовать входной последовательности, начиная с самого начала, с шаблоном.
								 //findМетод сканирует входную последовательность ищет следующие подпоследовательности , который соответствует шаблону.
        assertEquals(m.start(), 0); // указанный индекс ищет
